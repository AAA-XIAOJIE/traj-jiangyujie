...
def main():
    ...
    base=np.load(out/'runs/baseline/predictions.npz');summed=np.load(out/'runs/sum_pool/predictions.npz');direction=np.load(out/'runs/directional_sum/predictions.npz')
    ...
    metrics=pd.concat([pd.read_csv(out/'runs/baseline/metrics.csv'),pd.read_csv(out/'runs/sum_pool/metrics.csv').query("variant=='sum_pool'"),pd.read_csv(out/'runs/directional_sum/metrics.csv').query("variant=='directional_sum'")])
    summary=metrics.groupby(['split','variant'])[['ADE','FDE']].agg(['mean','std']).fillna(0);summary.columns=['_'.join(c) for c in summary.columns]
    summary.reset_index().to_csv(out/'tables/extended_summary.csv',index=False)
    pairs=[]
    ...
    (out/'audit/direction_effects.json').write_text(json.dumps(pairs,indent=2),encoding='utf8')
    # The curated gallery index is maintained separately from generated figures.
